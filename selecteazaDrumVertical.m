...
switch metodaSelectareDrum
    ...
    case 'greedy'
        ...
        for i = 2:size(d,1)
            linia = i;
            if d(i-1,2) == 1 %pixelul este localizat la marginea din stanga
                [~, poz] = min([E(i, d(i-1,2)), E(i, d(i-1,2) + 1)]);
                poz = poz - 1;
            elseif d(i-1,2) == size(E,2)%pixelul este la marginea din dreapta
                [~, poz] = min([E(i, d(i-1,2) - 1), E(i, d(i-1,2))]);
                poz = poz - 2;
            else
                [~, poz] = min([E(i, d(i-1,2) - 1), E(i, d(i-1,2)), E(i, d(i-1,2) + 1)]);
                poz = poz - 2;
            end
            coloana = d(i-1,2) + poz;
            d(i,:) = [linia coloana];
  ...
  E =  double(E);
        ...
        %parcurgem M de jos in sus si selectam drumul de cost minim
        liniaL = size(M, 1);
